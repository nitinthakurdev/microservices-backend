...
import { CustomError, winstonLogger } from "@nitinthakurdev/jobber-package";
...
export class GatewayServer {
    ...
    private errorHandler(app: Application): void {
        ...
        app.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
            ...
            if (isAxiosError(err)) {
                log.log('error', `GatewayService Axios Error - ${err?.response?.data?.comingFrom}:`,err?.response?.data);
                res.status(err?.response?.data?.statusCodes ?? DEFAULT_ERROR_CODE).json({ message: err?.response?.data?.message ?? 'Error occurred.' });
            }
